  ...
  return (
    <section className="card" aria-labelledby="local-ionosphere-title">
      ...
      <div className="sw-local-readings">
        ...
        <div><span>Station range</span><strong>{values.length ? `${Math.min(...values).toFixed(1)}–${Math.max(...values).toFixed(1)} TECU` : "Unavailable"}</strong></div>
      </div>
      ...
      <details className="sw-alert-bulletin">
        ...
        <div className="sw-table-scroll">
          <table className="sw-observation-table">
            ...
            <tbody>{rows.map(({ station, latest, status }) => <tr key={station.station}>
              <th scope="row">{station.name || station.station}</th>
              <td>{latest ? `${latest.vtec_tecu.toFixed(1)} TECU` : "Unavailable"}</td>
              <td>{observationTime(latest?.time)}</td><td>{status}</td>
            </tr>)}</tbody>
          </table>
        </div>
